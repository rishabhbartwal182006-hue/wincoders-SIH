 ...
 */
export function evaluateCondition(condition: Condition, state: SessionState): { matched: boolean; actualValue: unknown } {
  const actualValue = getNestedValue(state, condition.field);
  const targetValue = condition.value;

  if (actualValue === undefined || actualValue === null) {
    if (condition.operator === 'notEquals') return { matched: true, actualValue };
    if (condition.operator === 'exists') return { matched: false, actualValue };
 ...
 */
export function evaluateRule(rule: Rule, state: SessionState): TriggeredRule | null {
  ...
  for (const condition of rule.conditions) {
    ...
    matchedConditions.push(
      `Condition [${condition.field} ${condition.operator} ${JSON.stringify(condition.value)}]: matched with actual value = ${JSON.stringify(actualValue)}`
    );
  }
