...
export const DOMAIN_SCREENING_QUESTIONS: Record<string, DynamicQuestion> = {
 ...
 */
export function routeSymptomToQuestionSet(symptomName: string, site?: string): DynamicQuestion[] {
  const normSymptom = symptomName.toUpperCase();
  const normSite = site?.toUpperCase();

  const domainQuestions: DynamicQuestion[] = [];
  ...
  const combined: DynamicQuestion[] = [];
