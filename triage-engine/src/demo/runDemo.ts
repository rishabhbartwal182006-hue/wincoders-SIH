...
import { createServerInstance, ServerInstance } from "../server/httpServer.js";

import { SOCKET_EVENTS } from "../events/socketEvents.js";
import { DEMO_SCENARIOS, DemoScenario } from "./scenarios.js";
...
function parseArgs(): {
  selectedCase: "1" | "2" | "3" | "all";
  port: number;
} {
  const args = process.argv.slice(2);

  let selectedCase: "1" | "2" | "3" | "all" = "3";
  let port = 4000;
...
async function playScenario(
  ...
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(
        new Error(`Timed out while joining session ${scenario.sessionId}`),
      );
    }, 10000);

    kioskSocket.emit(
      SOCKET_EVENTS.CLIENT.JOIN_SESSION,
      {
  ...
  for (const step of scenario.steps) {
    ...
    console.log(
      `   📤 Emitting event: [${step.event.eventType}] (ID: ${step.event.id})`,
    );
  ...
  try {
    ...
    if (!auditResponse.ok) {
      console.log(
        `Unable to retrieve audit trail. HTTP ${auditResponse.status}`,
      );
    } else {
      const auditData = (await auditResponse.json()) as AuditResponse;
...
export async function runCliDemo(): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(
        new Error(`Unable to connect to MediKiosk server at ${serverUrl}`),
      );
    }, 10000);
  ...
  kioskSocket.on(SOCKET_EVENTS.SERVER.TRIAGE_UPDATED, (payload) => {
    console.log(
      `   👨‍⚕️ [Doctor Dashboard] Live Update: Session "${payload.sessionId}" -> [${payload.triageResult.triageLevel}]`,
    );
  ...
  kioskSocket.on(SOCKET_EVENTS.SERVER.QUESTIONNAIRE_INTERRUPTED, (payload) => {
    ...
    console.log(
      '       Guidance: "Please remain seated at the kiosk. Medical staff have been notified."',
    );
  });
  ...
  kioskSocket.on(SOCKET_EVENTS.SERVER.RED_FLAG_DETECTED, (payload) => {
    console.log(
      `   ⚠️ [RED FLAG] ${payload.previousLevel} → ${payload.currentLevel}`,
    );
  });
  ...
  if (selectedCase === "all") {
    ...
    const caseNum = Number(selectedCase) as 1 | 2 | 3;
  ...
  if (server) {
    try {
      ...
      console.warn(
        "⚠️ Warning while stopping MediKiosk server:",
